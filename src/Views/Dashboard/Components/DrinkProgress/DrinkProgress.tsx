    ...
    >
      ...
      <div style={progressMargin}>
        <label htmlFor="">
          ...
          {totalWater}/{dailyRequirement?.water}
          {FORM.ML}
        </label>
        <Progress.Line
          percent={
            totalWater > 0
              ? Math.floor(
                  (totalWater / (dailyRequirement?.water ?? 0)) * NUM.HUNDRED
                )
              : 0
          }
          status="active"
      ...
      <div style={progressMargin}>
        ...
        <Progress.Line
          percent={
            totalAlcohol > 0
              ? Math.floor(
                  (totalAlcohol / (dailyRequirement?.alcohol ?? 0)) *
                    NUM.HUNDRED
                )
              : 0
          }
      ...
      <div style={progressMargin}>
        ...
        <Progress.Line
          percent={
            totalCaffeine > 0
              ? Math.floor(
                  (totalCaffeine / (dailyRequirement?.caffeine ?? 0)) * 100
                )
              : 0
          }
          status="active"
