...
const UpdateDrink: React.FC<UpdateMealProps> = ({
  ...
  const formik = useFormik({
    ...
    validationSchema: Yup.object({
      drinkType: Yup.string().required(
        FORM_VALIDATION_MESSAGES().DRINK_SELECTOR
      ),
      container: Yup.string().required(
        FORM_VALIDATION_MESSAGES().CONTAINER_SELECT
      ),
      quantity: Yup.number()
  ...
  return (
    <>
      <CustomButton
        ...
        label={LABEL.CLOSE}
        onClick={() => setEditDrinkModal(false)}
      ></CustomButton>
      ...
      <form onSubmit={formik.handleSubmit}>
        <div className="input-group">
          ...
          <CustomSelect
            ...
            value={
              drinkTypeOptions.find(
                (opt) => opt.value === formik.values.drinkType
              ) || null
            }
            onChange={(selected) =>
              formik.setFieldValue("drinkType", selected?.value || "")
            }
          ...
          <CustomSelect
            ...
            value={
              containerOptions.find(
                (opt) => opt.value === formik.values.container
              ) || null
            }
            onChange={(selected) =>
              formik.setFieldValue("container", selected?.value || "")
            }
